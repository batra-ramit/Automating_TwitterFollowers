...
async function run() {
  ...
  await npage.type("input[name='email']", tempmail);
    ...
    await npage.waitFor(5000);
    // await npage.keyboard.press('Tab');
    await npage.keyboard.type(configJSO.password,{delay: 200 });
    ...
    await fpage.goto(configJSO.followurl);
    await npage.waitFor(5000);
    await fpage.waitForSelector("div[aria-label='Follow @sachin_rt']");
    await npage.waitFor(5000);
    await fpage.click("div[aria-label='Follow @sachin_rt']");
    await npage.waitFor(5000);
    await fpage.close();
    await page.close();
}
